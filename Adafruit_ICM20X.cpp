 ...
 */
bool Adafruit_ICM20X::begin_I2C(uint8_t i2c_addr , I2C_HandleTypeDef *i2c_handle, int32_t sensor_id) {
  (void)i2c_addr;
  (void)i2c_handle;
  (void)sensor_id;
  return false;
...
void Adafruit_ICM20X::_setBank(uint8_t bank_number) {
  ...
  writeRegisterByte(ICM20X_B0_REG_BANK_SEL, (bank_number & 0b11) << 4);
}
...
void Adafruit_ICM20X::setAccelRateDivisor(uint16_t new_accel_divisor) {
  ...
  writeRegister(ICM20X_B2_ACCEL_SMPLRT_DIV_1, (uint8_t *) &new_accel_divisor, 2);
  _setBank(0);
}
...
bool Adafruit_ICM20X::enableAccelDLPF(bool enable,
  ...
  if (!modifyRegisterMultipleBit(ICM20X_B2_ACCEL_CONFIG_1, cutoff_freq, 3, 3)) {
    return false;
  }
  return true;
}
...
void Adafruit_ICM20X::setInt1ActiveLow(bool active_low) {
  ...
  modifyRegisterBit(ICM20X_B0_REG_INT_PIN_CFG, true, 6);
  modifyRegisterBit(ICM20X_B0_REG_INT_PIN_CFG, active_low, 7);
}
/*!
